...
comment on column ACCOUNT_TYPE.TYPE_NAME is '계정코드 이름';
...
create table BUSINESS_INFO (
	...
	,BUSINESS_LOC_PROVINCE			nvarchar2(4)	not null
	,BUSINESS_LOC_CITY				nvarchar2(8)	not null
	,BUSINESS_LOC_DETAILED_ADDR		nvarchar2(50)	not null
	
	,HEADHQUARTER_LOC_PROVINCE		nvarchar2(4)	not null
	,HEADHQUARTER_LOC_CITY			nvarchar2(8)	not null
	,HEADHQUARTER_LOC_DETAILED_ADDR	nvarchar2(50)	not null
...
--아래는 내용 속성 예시. alter table drop column //add column 으로 속성을 필요한 만큼 알아서 넣어 쓰기. (사실 프로토타입이라 그냥 쓰다가 나중에 협의한 내용 + 알아서 넣는 속성으로 해도 됨)
